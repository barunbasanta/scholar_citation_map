...
def scholar_publications(profile_url: str, publication_limit: int) -> list[dict]:
    """
...
def match_openalex_work(title: str, api_key: str) -> dict | None:
    data = oa_get(
        "/works",
        {"search": title, "per-page": 5, "select": "id,display_name,cited_by_count"},
        api_key,
    )
...
def institution_location(inst: dict) -> dict | None:
    ...
    place = city.strip() or display_name.strip() or country.strip()
...
def extract_locations_from_citing_work(work: dict) -> list[dict]:
    found = {}
    for authorship in work.get("authorships", []):
        for inst in authorship.get("institutions", []):
            loc = institution_location(inst)
            if not loc:
                continue
            key = (
                round(loc["lat"], 4),
...
def citing_works_for_openalex_id(
    ...
    data = oa_get(
        "/works",
        {
            ...
            "per-page": min(per_work_limit, 200),
...
def aggregate_citation_locations(
    ...
    for index, publication in enumerate(publications, start=1):
        ...
        for citing_work in citing_works:
            locations = extract_locations_from_citing_work(citing_work)
    ...
    if not profile_url.strip() or not origin_text.strip() or not openalex_key.strip():
        ...
        try:
            ...
            with st.spinner("Reading publication metadata from Google Scholar…"):
                publications = scholar_publications(profile_url, publication_limit)
...
if "locations_df" in st.session_state:
    ...
    if locations_df.empty:
        st.warning(
            ...
            "Try increasing the limits or checking the OpenAlex matches."
        )
    else:
        m1, m2, m3, m4 = st.columns(4)
        m1.metric("Origin", origin["label"])
        m2.metric("Mapped places", f"{len(locations_df):,}")
        m3.metric("Countries", f"{locations_df['Country'].nunique():,}")
        m4.metric("Mapped citing papers", f"{int(locations_df['Citing papers'].sum()):,}")
        ...
        with st.expander("OpenAlex publication matching audit"):
